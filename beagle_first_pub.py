...
def main(args=None):
    ...
    action_client = ActionClient(node, Distbeagle, 'beagle_action')
    ...
    if result.goal_status.status == result.goal_status.STATUS_SUCCEEDED:
        node.get_logger().info('Action completed successfully')
    else:
        node.get_logger().info('Action failed')
